...
interface ModalExpendituresProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function ModalExpenditures({ isOpen, onClose, onSuccess }: ModalExpendituresProps) {
  const user = authService.getUser();
  const { createExpenditure, isCreating } = useExpenditures();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createExpenditure(
      { name: form.name, price: Number(form.price), note: form.note, userId: user.id },
      {
        onSuccess: () => {
          onSuccess();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-lg font-bold text-gray-900">Tambah Pengeluaran</h2>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2.5 border border-gray-200 text-gray-600 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors">
              Batal
            </button>
            <button type="submit" disabled={isCreating} className="flex-1 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium disabled:opacity-50 transition-colors">
